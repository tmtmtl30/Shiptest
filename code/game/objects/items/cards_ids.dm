...
/obj/item/card/id/attackby(obj/item/W, mob/user, params)
	if(istype(W, /obj/item/holochip))
		insert_money(W, user)
		return
	else if(istype(W, /obj/item/spacecash/bundle) || istype(W, /obj/item/coin))
...
/obj/item/card/id/proc/insert_money(obj/item/I, mob/user, physical_currency)
	...
	var/item_value = registered_account.absorb_cash(I)
	if(!item_value)
		to_chat(user, "<span class='warning'>[I] doesn't seem to be worth anything!</span>")
		return

	// deletion is handled by absorb_cash()
...
/obj/item/card/id/AltClick(mob/living/user)
	...
	var/obj/item/holochip/holochip = registered_account.create_holochip(user.drop_location(), amount_to_remove)
	if(holochip)
