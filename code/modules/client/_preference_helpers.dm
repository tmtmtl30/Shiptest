		...
		if(!(pref.type in pref_value_list))
			...
			if(!pref_avail)
				...
				if(pref_value_list[pref.type] != PREFERENCE_ENTRY_UNAVAILABLE)
					return "Preference [pref.type] was present in the value list and had a value other than \"unavailable\"!"
			else
...
/proc/apply_prefs_list_to_human(mob/living/carbon/human/H, list/pref_value_list)
...
/proc/get_randomized_pref_list(list/preset_list, rand_hints)
	...
	for(var/datum/preference/pref as anything in GLOB.pref_topo_order)
		...
		if(pref.type in preset_list)
			...
			var/list/pref_rand_deps = assemble_pref_dep_list(pref, preset_list, TRUE)
